...
int nfs_mkdir(const char* path, mode_t mode) {
	...
	char* fname  = nfs_get_fname(path);

	dentry = new_dentry(fname, NFS_DIR_FILE); 
...
int nfs_getattr(const char* path, struct stat * nfs_stat) {
	...
	nfs_stat->st_blksize = NFS_BLOCK_SZ();

	if (is_root) {
		nfs_stat->st_size	= super.sz_usage; 
		nfs_stat->st_blocks = NFS_DISK_SZ() / NFS_BLOCK_SZ();
...
int main(int argc, char **argv)
{
	...
	nfs_options.device = strdup("/home/students/210110621/ddriver");
